...
from opportunity_radar.matching.analysis import AnalysisPolicy, NullAnalysisAdapter
...
def test_a_policy_override_reaches_the_adapter() -> None:
    """scripts/eval_analysis.py needs every case to reach the model, whatever the
    default production policy would skip."""
    settings = _settings(ai_enabled=True, groq_api_key="k")
    override = AnalysisPolicy(skip_verdicts=frozenset(), skip_ineligible=False)

    adapter = build_analysis_adapter(settings, _engine(), policy=override)

    assert isinstance(adapter, GroqAnalysisAdapter)
    assert adapter._policy is override
