...
- [ ] `grep -rni ollama src scripts tests compose*.yaml Makefile apps/web/src` não retorna nada.
- [ ] `/health` tem a chave `ai` e não tem `ollama`.
- [ ] `npm run check` em `apps/web` passa.
