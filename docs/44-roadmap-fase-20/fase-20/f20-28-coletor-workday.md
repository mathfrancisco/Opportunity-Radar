...
- Este card é o sub-card de **Workday** do F17-10. Aplicar o escopo e os critérios abaixo só a Workday.
...
| Ação | Caminho | O quê |
| --- | --- | --- |
| Criar | `docs/pesquisas/termos-workday.md` | Revisão dos termos de uso do endpoint público do Workday, antes de qualquer código. Se o endpoint proibir automação, este documento é o entregável final do sub-card e ele fecha como "não viável". |
| Criar | `src/opportunity_radar/acquisition/workday.py` | Coletor do Workday, no formato de `src/opportunity_radar/acquisition/lever.py` (`LeverCollector`: `source_type`, `capabilities`, `healthcheck`, `discover`, retentativa com `Retry-After`, `validate_*`). |
| Alterar | `src/opportunity_radar/acquisition/registry.py` | `build_collector_registry` (linhas 12-26) monta a tupla de coletores do worker e da API; adicionar `WorkdayCollector()`. |
| Alterar | `src/opportunity_radar/acquisition/probing.py` | `PROBE_TYPES` (linha 24) e `PUBLIC_ENDPOINT_REFERENCES` (linhas 25-30) só cobrem `ashby`, `lever`, `greenhouse`, `remotive`; adicionar `"workday"` e a referência ao endpoint público. `probe_request` (linhas 43-62) precisa de um novo `elif source_type == "workday"` que monte `company_reference` a partir de `configuration["tenant_identifier"]`. |
| Alterar | `src/opportunity_radar/acquisition/proposals.py` | `IDENTIFIER_KEYS` (linhas 22-26) só mapeia `ashby`/`lever`/`greenhouse`; adicionar `"workday": "tenant_identifier"` — nome de campo a confirmar na revisão de termos. |
| Alterar | `src/opportunity_radar/companies/registration.py` | `SUPPORTED_ATS` (linha 48) e o dicionário de `validators` em `_source_values` (linhas 373-376) só cobrem os três ATS atuais; adicionar `"workday"` e `WorkdayCollector.validate_tenant_identifier` (ou o nome que a revisão de termos confirmar). |
| Alterar | `apps/web/src/components/SourceCreateForm.tsx` | `configFields` (linhas 21-58) e `typeLabels` (linhas 60-66) só têm entradas para `ashby`/`lever`/`greenhouse`/`remotive`/`manual`; adicionar `workday` com o campo `tenant_identifier` (obrigatório) e `company_name` (opcional), e o rótulo `"Workday"`. |
| Alterar | `apps/web/src/features/sources/api.ts` | `sourceTypes` (linhas 204-205) é a lista fechada de `SourceType`; adicionar `"workday"`. |
| Criar | `tests/backend/acquisition/test_workday_collector.py` | Testes do coletor contra um board falso, no formato de `tests/backend/acquisition/test_lever_collector.py`. |
| Criar | `tests/e2e/fake_workday_board.py` | Servidor falso do Workday (paginação, erro, 429), no formato de `tests/e2e/fake_job_board.py`. |

Endpoint provável do Workday: busca JSON por tenant e site (`/wday/cxs/<tenant>/<site>/jobs`), paginada, com detalhe por vaga — **a confirmar na revisão de termos**, não é fato até lá.

Workday identifica o board por **dois** valores (tenant e site), não um só; `company_reference` pode precisar codificar `"<tenant>/<site>"` ou o coletor pode aceitar um segundo campo (`site_identifier`, já usado pelo Lever para outro fim) — decidir na revisão de termos, não aqui.

## Interfaces

```python
# src/opportunity_radar/acquisition/workday.py
class WorkdayCollector:
    """Lê vagas públicas do Workday. Nomes de campo e forma do endpoint dependem
    da revisão de termos em docs/pesquisas/termos-workday.md."""
    ...
    async def discover(
        ...
        """paginação por `offset`/`limit` (ou cursor equivalente), a confirmar na revisão de termos. Falha nunca deve parecer board vazio: página curta só
        conta como fim quando o coletor sabe que é o fim (mesmo contrato do Lever,
        linha 138-142), não só um retorno vazio."""

    @staticmethod
    def validate_tenant_identifier(value: str) -> str:
        """Formato do identificador a confirmar na revisão de termos."""
```

## Passos

1. Escrever a revisão de termos em `docs/pesquisas/termos-workday.md` antes de qualquer código: o que o endpoint público permite, exige atribuição, limite de taxa, e se proíbe automação. Se proibir, parar aqui e fechar o sub-card como "não viável", registrando o motivo no card.
...
3. Escrever os testes do coletor (`tests/backend/acquisition/test_workday_collector.py`) contra o board falso, cobrindo item válido, paginação, schema alterado e erro HTTP — antes do código do coletor.
4. Criar `tests/e2e/fake_workday_board.py`, no formato de `tests/e2e/fake_job_board.py`, com paginação e um caminho de erro (404, 429 com `Retry-After`, 500).
5. Implementar `src/opportunity_radar/acquisition/workday.py`, seguindo a forma de `LeverCollector` (`source_type`, `capabilities`, retentativa com `Retry-After`, classificação de erro HTTP em `AcquisitionErrorCode`, `validate_tenant_identifier`).
6. Registrar o coletor em `build_collector_registry` (`registry.py`, linhas 12-26).
7. Adicionar `"workday"` a `PROBE_TYPES` e `PUBLIC_ENDPOINT_REFERENCES` em `probing.py`, e o ramo correspondente em `probe_request`.
8. Adicionar `"workday": "tenant_identifier"` a `IDENTIFIER_KEYS` em `proposals.py`.
9. Adicionar `"workday"` a `SUPPORTED_ATS` e o validador correspondente em `registration.py`.
10. Adicionar o tipo ao formulário (`SourceCreateForm.tsx`) e a `sourceTypes` (`api.ts`).
11. Mapear departamento (F20-03) e senioridade (F20-02) só para os campos que o endpoint do Workday realmente expõe — não inventar correspondência para campo ausente.
...
- `tests/backend/acquisition/test_workday_collector.py::test_parses_listed_jobs_and_preserves_payload`
- `tests/backend/acquisition/test_workday_collector.py::test_paginates_until_short_page`
- `tests/backend/acquisition/test_workday_collector.py::test_retries_rate_limit_using_retry_after`
- `tests/backend/acquisition/test_workday_collector.py::test_classifies_http_errors`
- `tests/backend/acquisition/test_workday_collector.py::test_rejects_invalid_identifier_and_schema`
- `tests/backend/acquisition/test_workday_collector.py::test_skips_malformed_listed_job_and_reports_it`
- `tests/backend/acquisition/test_service.py::test_probe_recognizes_workday_source_type`
- `tests/backend/companies/test_importer.py::test_registration_accepts_workday_source_type` (ou equivalente em `test_domain.py`, conforme onde `SUPPORTED_ATS` for exercitado)
...
```bash
docker compose -p f20-28 -f compose.yaml -f compose.dev.yaml run --rm api pytest -q tests/backend/acquisition/test_workday_collector.py tests/backend/acquisition/test_service.py
