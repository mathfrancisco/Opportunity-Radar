...
- Este card é o sub-card de **Teamtailor** do F17-10. Aplicar o escopo e os critérios abaixo só a Teamtailor.
...
| Ação | Caminho | O quê |
| --- | --- | --- |
| Criar | `docs/pesquisas/termos-teamtailor.md` | Revisão dos termos de uso do endpoint público do Teamtailor, antes de qualquer código. Se o endpoint proibir automação, este documento é o entregável final do sub-card e ele fecha como "não viável". |
| Criar | `src/opportunity_radar/acquisition/teamtailor.py` | Coletor do Teamtailor, no formato de `src/opportunity_radar/acquisition/lever.py` (`LeverCollector`: `source_type`, `capabilities`, `healthcheck`, `discover`, retentativa com `Retry-After`, `validate_*`). |
| Alterar | `src/opportunity_radar/acquisition/registry.py` | `build_collector_registry` (linhas 12-26) monta a tupla de coletores do worker e da API; adicionar `TeamtailorCollector()`. |
| Alterar | `src/opportunity_radar/acquisition/probing.py` | `PROBE_TYPES` (linha 24) e `PUBLIC_ENDPOINT_REFERENCES` (linhas 25-30) só cobrem `ashby`, `lever`, `greenhouse`, `remotive`; adicionar `"teamtailor"` e a referência ao endpoint público. `probe_request` (linhas 43-62) precisa de um novo `elif source_type == "teamtailor"` que monte `company_reference` a partir de `configuration["company_identifier"]`. |
| Alterar | `src/opportunity_radar/acquisition/proposals.py` | `IDENTIFIER_KEYS` (linhas 22-26) só mapeia `ashby`/`lever`/`greenhouse`; adicionar `"teamtailor": "company_identifier"` — nome de campo a confirmar na revisão de termos. |
| Alterar | `src/opportunity_radar/companies/registration.py` | `SUPPORTED_ATS` (linha 48) e o dicionário de `validators` em `_source_values` (linhas 373-376) só cobrem os três ATS atuais; adicionar `"teamtailor"` e `TeamtailorCollector.validate_company_identifier` (ou o nome que a revisão de termos confirmar). |
| Alterar | `apps/web/src/components/SourceCreateForm.tsx` | `configFields` (linhas 21-58) e `typeLabels` (linhas 60-66) só têm entradas para `ashby`/`lever`/`greenhouse`/`remotive`/`manual`; adicionar `teamtailor` com o campo `company_identifier` (obrigatório) e `company_name` (opcional), e o rótulo `"Teamtailor"`. |
| Alterar | `apps/web/src/features/sources/api.ts` | `sourceTypes` (linhas 204-205) é a lista fechada de `SourceType`; adicionar `"teamtailor"`. |
| Criar | `tests/backend/acquisition/test_teamtailor_collector.py` | Testes do coletor contra um board falso, no formato de `tests/backend/acquisition/test_lever_collector.py`. |
| Criar | `tests/e2e/fake_teamtailor_board.py` | Servidor falso do Teamtailor (paginação, erro, 429), no formato de `tests/e2e/fake_job_board.py`. |

Endpoint provável do Teamtailor: página pública de vagas por empresa, com feed — **a confirmar na revisão de termos**, não é fato até lá.

## Interfaces

```python
# src/opportunity_radar/acquisition/teamtailor.py
class TeamtailorCollector:
    """Lê vagas públicas do Teamtailor. Nomes de campo e forma do endpoint dependem
    da revisão de termos em docs/pesquisas/termos-teamtailor.md."""
    ...
    async def discover(
        ...
        """o feed pode não paginar por página numerada; confirmar o mecanismo (feed completo vs. paginado) na revisão de termos. Falha nunca deve parecer board vazio: página curta só
...
1. Escrever a revisão de termos em `docs/pesquisas/termos-teamtailor.md` antes de qualquer código: o que o endpoint público permite, exige atribuição, limite de taxa, e se proíbe automação. Se proibir, parar aqui e fechar o sub-card como "não viável", registrando o motivo no card.
...
3. Escrever os testes do coletor (`tests/backend/acquisition/test_teamtailor_collector.py`) contra o board falso, cobrindo item válido, paginação, schema alterado e erro HTTP — antes do código do coletor.
4. Criar `tests/e2e/fake_teamtailor_board.py`, no formato de `tests/e2e/fake_job_board.py`, com paginação e um caminho de erro (404, 429 com `Retry-After`, 500).
5. Implementar `src/opportunity_radar/acquisition/teamtailor.py`, seguindo a forma de `LeverCollector` (`source_type`, `capabilities`, retentativa com `Retry-After`, classificação de erro HTTP em `AcquisitionErrorCode`, `validate_company_identifier`).
6. Registrar o coletor em `build_collector_registry` (`registry.py`, linhas 12-26).
7. Adicionar `"teamtailor"` a `PROBE_TYPES` e `PUBLIC_ENDPOINT_REFERENCES` em `probing.py`, e o ramo correspondente em `probe_request`.
8. Adicionar `"teamtailor": "company_identifier"` a `IDENTIFIER_KEYS` em `proposals.py`.
9. Adicionar `"teamtailor"` a `SUPPORTED_ATS` e o validador correspondente em `registration.py`.
10. Adicionar o tipo ao formulário (`SourceCreateForm.tsx`) e a `sourceTypes` (`api.ts`).
11. Mapear departamento (F20-03) e senioridade (F20-02) só para os campos que o endpoint do Teamtailor realmente expõe — não inventar correspondência para campo ausente.
...
- `tests/backend/acquisition/test_teamtailor_collector.py::test_parses_listed_jobs_and_preserves_payload`
- `tests/backend/acquisition/test_teamtailor_collector.py::test_paginates_until_short_page`
- `tests/backend/acquisition/test_teamtailor_collector.py::test_retries_rate_limit_using_retry_after`
- `tests/backend/acquisition/test_teamtailor_collector.py::test_classifies_http_errors`
- `tests/backend/acquisition/test_teamtailor_collector.py::test_rejects_invalid_identifier_and_schema`
- `tests/backend/acquisition/test_teamtailor_collector.py::test_skips_malformed_listed_job_and_reports_it`
- `tests/backend/acquisition/test_service.py::test_probe_recognizes_teamtailor_source_type`
- `tests/backend/companies/test_importer.py::test_registration_accepts_teamtailor_source_type` (ou equivalente em `test_domain.py`, conforme onde `SUPPORTED_ATS` for exercitado)
...
```bash
docker compose -p f20-29 -f compose.yaml -f compose.dev.yaml run --rm api pytest -q tests/backend/acquisition/test_teamtailor_collector.py tests/backend/acquisition/test_service.py
docker compose -p f20-29 -f compose.yaml -f compose.dev.yaml run --rm api ruff check .
docker compose -p f20-29 -f compose.yaml -f compose.dev.yaml run --rm api mypy
```
